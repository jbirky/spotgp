"""BlackJAX NUTS sampler for GP hyperparameters."""
...
class BlackJAXSampler(MCMCSampler):
    ...
    def _run_pathfinder_warmup(self, rng_key, theta_inits, log_posterior_fn,
        ...
        logger.info(f"Pathfinder warmup: {n_paths} paths, "
        ...
        for i in range(n_paths):
            ...
            logger.info(f"  Path {i}: ELBO = {float(state.elbo):.2f}")
        ...
        logger.info(f"  Best path: {best_idx} (ELBO = {float(best.elbo):.2f})")
        ...
        logger.info(f"  Adapted step size: {step_size:.6f}")
        logger.info(f"  Inv mass diag range: [{float(inv_mass_diag.min()):.2e}, "
    ...
    def run_warmup(self, n_warmup=500, theta_init=None,
        ...
        if warmup_method == "pathfinder":
            ...
            logger.info(f"Warmup: {n_warmup} steps (dual averaging, fixed mass matrix, "
            ...
            logger.info(f"  Adapted step size: {float(adapted_step_size):.6f}")
            ...
            logger.info(f"Warmup: {n_warmup} steps (window adaptation, "
                  f"init step_size={step_size:.6f})...")
        ...
        if self._checkpoint_file is not None:
            ...
            logger.info("  Warmup checkpoint saved; clearing warmup memory...")
    ...
    def run_sampling(self, n_samples=1000):
        ...
        if n_chains > 1:
            ...
            logger.info(f"Sampling {n_samples} iterations x {n_chains} chains "
            ...
            if is_multi_chain_state:
                ...
                if _per_chain_inits is not None and _per_chain_inits.shape[0] >= n_chains:
                    ...
                    logger.info(f"  Using {n_chains} distinct MAP solutions as chain init positions")
                else:
            ...
            logger.info(f"NUTS complete: {n_chains} chains x {n_samples} samples, "
                  f"{total_div} total divergences, "
                  f"mean acceptance rate = {mean_accept:.3f}")
        else:
            logger.info(f"Sampling {n_samples} post-warmup iterations...")
            ...
            logger.info(f"NUTS complete: {n_samples} samples, "
    ...
    def save_checkpoint(self, path=None, append_samples=True,
        ...
        logger.debug(f"Checkpoint saved to {path} ({n_on_disk} samples on disk)")
        ...
        if plot_corner and n_on_disk > 0:
            ...
            plt.close(fig)
            logger.debug(f"Corner plot saved to {corner_path} "
                  f"({n_on_disk} samples)")
    ...
    def load_checkpoint(self, checkpoint_file=None):
        ...
        if "all_theta_maps" in data:
            ...
            logger.info(f"Restored {len(self.all_theta_maps)} MAP solutions "
                  f"from checkpoint")
    ...
    def run_smc(self, n_particles=500, n_mcmc_steps=10,
        ...
        if (hasattr(self, 'all_theta_maps')
            ...
            logger.info(f"Initialized {n_particles} particles from Laplace "
                  f"approximation around {n_maps} MAP solution(s)")
        else:
            particles = (jax.random.uniform(init_key,
                                             shape=(n_particles, gp.n_params))
                         * (hi - lo) + lo)
            logger.info(f"Initialized {n_particles} particles from uniform prior")
        ...
        def _save_smc_checkpoint(smc_st, lambdas_, step_sizes_,
            ...
            logger.debug(f"  Checkpoint saved to {chk_path} "
                  f"(lambda={float(smc_st.tempering_param):.6f})")

        # --- Run tempering loop ---------------------------------------
        if use_batched:
            logger.info(f"SMC: {n_particles} particles, "
        ...
        for step in range(max_tempering_steps):
            ...
            if np.isfinite(ll_inc):
                ...
                logger.warning(f"  Warning: non-finite log_likelihood_increment "
                      f"({ll_inc}) at step {step + 1}, skipping")

            logger.info(f"  Step {step + 1}: lambda={lam:.6f}, "
            ...
            logger.warning(f"  Warning: reached max_tempering_steps="
        ...
        logger.info(f"SMC complete: {n_steps} tempering steps, "
