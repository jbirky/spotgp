...
keys_all = keys_trend + keys_cos + keys_sin + keys_cos2
...
class TrainKernel(object):
    ...
    def __init__(self, hyperparam, tsim=1000, tsamp=0.1, nsim=5e2, tcut=100, 
                 fit_ft=False, log_power=False, log_params=False, verbose=True, 
                 fit_orders=[0,1,2], fit_sin=True, opt_bounds={}):
    ...
    def fit_kernel_trend(self, p0=None):
        ...
        bounds = get_p0(self.opt_bounds, keys_trend)
    ...
    def fit_kernel_cos(self, trend_param, p0=None):
        ...
        bounds = get_p0(self.opt_bounds, keys_cos)
    ...
    def fit_kernel_cos_second(self, trend_param, cos_param, p0=None):
        ...
        bounds = get_p0(self.opt_bounds, keys_cos2)
    ...
    def fit_kernel_sin(self, trend_param, cos_param, p0=None):
        ...
        bounds = get_p0(self.opt_bounds, keys_sin)

        result = minimize(self.loss_sin, p0, args=(trend_param, cos_param), method="trust-constr", bounds=bounds)
        
        return result.x

    def fit_kernel_sin_cos(self, trend_param, p0=None):

        bounds = get_p0(self.opt_bounds, keys_sin + keys_cos)
        result = minimize(self.loss_sin_cos, p0, args=(trend_param), method="trust-constr", bounds=bounds)
    ...
    def fit_model(self, pinit):
        ...
        if 1 in self.fit_orders:
            ...
            if self.fit_sin == True:
                ...
                plist.append(res_sin_cos)
                pdict.update(res_sin_cos)
            else:
                res_cos = dict(zip(keys_cos, pcos))
                plist.append(res_cos)
                pdict.update(res_cos)
        ...
        if 2 in self.fit_orders:
            ...
            plist.append(res_cos2)
            pdict.update(res_cos2)
    ...
    def predict_components(self, pfit):
        ...
        if 1 in self.fit_orders:
            if self.fit_sin == True:
                ...
                pcos = get_p0(pfit, keys_cos)
                ysin_cos = self.kernel_cos(self.xdata, pcos)
        else:
    ...
    def plot_kernel_fit(self, ypred, ypred2=None, 
        ...
        for ii in range(int(round(self.tcut / self.peq))):
        ...
        if tlag_rng is None:
            ...
            axes[0].set_xlim(min(tlag_rng), max(tlag_rng))
        ...
        for ii, text in enumerate(text_blocks2):
            axes[1].text(0.88-.15*ii, 0.97, text, transform=plt.gca().transAxes, va="top", fontsize=22, color='blue')
